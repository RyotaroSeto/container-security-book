...
```
- ベースイメージのセキュリティ
  - FROM nginxのようなベースイメージを指定するとDocker Hubのオフィシャルイメージが使用される
  - タグを使用しない場合`latest`バージョンが指定されるため、ベースイメージに含まれるミドルウェア等に非互換な機能が入っている場合、正常に機能しなくなる
  - イメージはタグだけでなく、nginx@sha256:61faf.... のようにダイジェストを指定できる
  - ベースイメージを指定する際はできるだけダイジェストを指定するのがよい。

- Distrolessイメージを使用した方が良い
